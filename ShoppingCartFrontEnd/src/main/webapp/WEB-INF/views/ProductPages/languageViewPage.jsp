...
<div class="container">
      <c:forEach var="listlanguage" items="${languagesList}">
     
      <c:if test="${listlanguage.getLanguage()== language}">
      <c:set var="languages" value="${listlanguage}"/>
      </c:if>
      </c:forEach>   
      
     
      <c:forEach var="listValue" items="${languages.getProducts()}">

...
<div class="container">
      <c:forEach var="listauthors" items="${authorsList}">
     
      <c:if test="${listauthors.getAuthor()== author}">
      <c:set var="authors" value="${listauthors}"/>
      </c:if>
      </c:forEach>   
      
     
      <c:forEach var="listValue" items="${authors.getProducts()}">
